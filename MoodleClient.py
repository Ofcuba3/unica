...
class MoodleClient(object):
    ...
    def getEvidences(self):
        ...
        for n in nodes:
            ...
            nfilelist = []
            for f in nodefiles:
                directurl = str(f['href']).replace('pluginfile.php','webservice/pluginfile.php')
    ...
    def upload_file(self,file,evidence,itemid=None):
        fileurl = self.path + 'admin/tool/lp/user_evidence_edit.php?id='+evidence['id']+'&userid='+self.userid+'&return=list'
    ...
    def upload_save(self,file_name,evidence,itemid):
        fileid = self.upload_file(file_name,evidence)
        evidence = self.saveEvidence(evidence)
        return fileid
...
if loged:
   evidence = client.createEvidence('')
   sleep(5)
   client.upload_file('',evidence,0)'''
